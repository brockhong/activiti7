...
public class ActivitiUtil {
    ...
    @Autowired
    private MyProcessRuntimeImpl myProcessRuntime;
...
//                .processDefinitions(Pageable.of(startNum, endNum));
        return activitiUtil.myProcessRuntime.processDefinitions(Pageable.of(startNum, endNum));
...
//                        TaskPayloadBuilder.claim().withTaskId(task.getId()).build());
     ...
     * @throws Exception
     */
    public static void auditByCandidate1(String assignee, boolean auditFlg, HashMap variables) throws Exception {
...
//                logger.info(assignee + "拾取任务成功");
     ...
     * @throws Exception
     */
    private static void backProcessMultiInstance(org.activiti.api.task.model.Task task) throws Exception {
        ...
        // list里的第二条代表上一个任务  下面的方法中随便取一个就可以哈哈哈
        HistoricTaskInstance lastTask = htiList.get(1);

        List<HistoricTaskInstance> lastTasks = new ArrayList<>();
        BpmnModel bpmnModel = activitiUtil.repositoryService.getBpmnModel(task.getProcessDefinitionId());
        ...
        for (SequenceFlow incomingFlow : incomingFlows) {
            //获取输入节点元素
            FlowElement targetFlowElement = incomingFlow.getSourceFlowElement();
            //排除非用户任务接点
            if (targetFlowElement instanceof UserTask) {
                //获取输出节点id==名称
//                System.out.println(incomingFlow.getTargetFlowElement().getId()+"==="
//                        +incomingFlow.getTargetFlowElement().getName());

                UserTask userTask = (UserTask) targetFlowElement;
                MultiInstanceLoopCharacteristics loopCharacteristics = userTask.getLoopCharacteristics();
                if (loopCharacteristics.getCompletionCondition() != null) {
                    logger.info("==并行节点==" + loopCharacteristics);
                    a=loopCharacteristics.getInputDataItem();
                    logger.info("==并行节点==" + loopCharacteristics.getInputDataItem());
                    a=a.substring(a.indexOf("{")+1,a.indexOf("}"));

                    for (HistoricTaskInstance historicTaskInstance : htiList) {
                        if (historicTaskInstance.getId().equals(
                                incomingFlow.getSourceFlowElement().getId())) {
                            lastTasks.add(historicTaskInstance);
                        }
                    }
                }
        ...
        lastTasks) {
            signList.add(historicTaskInstance.getAssignee());
        }
...
//                        .withVariables(map).build());
        ...
        // 完成任务
        activitiUtil.taskService.complete(task.getId(),map);
...
//        }
    ...
    public static void rejection(String assignee) throws Exception {
     ...
     * @throws Exception
     */
    private static void backProcessExclusive(org.activiti.api.task.model.Task task) throws Exception {
        ...
        // list里的第二条代表上一个任务   htiList.get(1); 也能取到
        HistoricTaskInstance lastTask = null; //htiList.get(1);
...
//        List<HistoricTaskInstance> lastTasks = new ArrayList<>();
        ...
        for (SequenceFlow incomingFlow : incomingFlows) {
            ...
            if(targetFlowElement instanceof ExclusiveGateway){
                ...
                FlowNode flowNode1 =(FlowNode) bpmnModel.getFlowElement(((ExclusiveGateway)targetFlowElement).getId());

                List<SequenceFlow> incomingFlows1 =  flowNode1.getIncomingFlows();
                for (SequenceFlow  gataway : incomingFlows1) {
                    //获取输入节点元素
                    FlowElement targetFlowElement1 =  gataway.getSourceFlowElement();

                    if(targetFlowElement1 instanceof  UserTask){
                        UserTask userTask = (UserTask) targetFlowElement1;
                        for (HistoricTaskInstance historicTaskInstance :htiList) {
                            logger.info("======="+userTask.getId()+"===="+historicTaskInstance.getTaskDefinitionKey());
                            if(historicTaskInstance.getTaskDefinitionKey().equals(userTask.getId())){
                                //获取最后一次流程
        ...
        //流程走向 删除回退的历史 展示高亮 activitHistoryController
        //act_hi_taskinst
        activitiUtil.historyService.deleteHistoricTaskInstance(task.getId());
        //act_hi_actinst
        ProcessEngine defaultProcessEngine = ProcessEngines.getDefaultProcessEngine();
        ManagementService managementService = defaultProcessEngine.getManagementService();

        final String id = task.getId();
        managementService.executeCustomSql( new AbstractCustomSqlExecution<HisActInstanceMapper,
               Integer>(HisActInstanceMapper.class){
            public Integer execute(HisActInstanceMapper myCustomMapper) {
                return myCustomMapper.deletetaskId(id);
            }
        });
        System.out.printf("====================activiti=======================");
        //todo 自己扩展
    }
